...
//TRACER_CALL(t,"makeMainList");
  ...
  { const char *group="Filter Bank Parameters";
    ...
    main_list->append(OPERATOR_NEW GUIInputParameter<int>(
      product_filter_order,"product_filter_order",1,INT_MAX,group) );
...
//TRACER_CALL(t,"checkMainInput");
  if (product_filter_order<scaling_function_order) {
    cerr << "must have product_filter_order >= scaling_function_order"
         << endl;
    product_filter_order = scaling_function_order;
  }
...
void runMain(bool /*called_before*/) {
  ...
  SplineFilterBank sfb(scaling_function_order,product_filter_order);
  const Filter &lowpass=sfb.lowpassFilter();
  const Filter &dual_lowpass=sfb.dualLowpassFilter();
...
//  for (int i=fvalues.firstIndex();i<=fvalues.lastIndex();i++) {
//    cout << "\ti,f = " << i << " " << fvalues.value(i) << endl;
//  }
...
//  for (int i=phi_tilde_values->firstIndex();
//  i<=phi_tilde_values->lastIndex();i++) {
//    cout << "\ti,phi_tilde = " << i << " " << phi_tilde_values->value(i)
//         << endl;
//  }
...
//  for (int i=gamma.firstIndex();i<=gamma.lastIndex();i++) {
//    cout << "\ti,gamma = " << i << " " << gamma.value(i) << endl;
//  }
