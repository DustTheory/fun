...
number_points(q1.numberPoints()*q2.numberPoints()) {
  ...
  CHECK_TEST(abs(sum-1.)<DBL_EPSILON*static_cast<double>(number_points));
#endif
}
...
int dimensionalRoot(int dim,int n) {
  switch (dim) {
    case 1:
      return n;
    case 2: {
      int i=static_cast<int>(nearbyint(sqrt(static_cast<double>(n))));
      CHECK_SAME(i*i,n);
      return i;
    }
    case 3: {
      int i=static_cast<int>(nearbyint(pow(static_cast<double>(n),1./3.)));
      CHECK_SAME(i*i*i,n);
      return i;
    }
    default:
...
int triangleRoot(int n) {
  int i=static_cast<int>(
        nearbyint((sqrt(1.+8.*static_cast<double>(n))-1.)*0.5));
...
int tetrahedronRoot(int n) {
  double n3=static_cast<double>(3*n);
  double s=sqrt(n3*n3-1./27.);
  double A=pow(n3+s,1./3.);
  double B=pow(n3-s,1./3.);
  double a=A+B-1.;
...
Quadrature<1>(n),np_1d(n) {
  ...
  double w=this->quadrature_weights[0]=this->quadrature_weights[n-1]
    =1./static_cast<double>(n*(n-1));
  if (n>2) {
...
: Quadrature<1>(n),np_1d(n) {
  CHECK_TEST(n>=2);
  int N=n-1;
  double dtheta=0.5*M_PI/static_cast<double>(N);
  this->quadrature_points[0]=0.;
  double theta=dtheta;
  ...
  for (int k=0;k<=M;k++) {
    double sum=0.;
    for (int j=1;j<M;j++) {
      sum+=cos(static_cast<double>(j*k)*dtheta)/static_cast<double>(1-4*j*j);
    }
    sum=2.*sum+1.;
    if (M>0) {
      double term=cos(M*k*dtheta)/static_cast<double>(1.-4*M*M);
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
#if (SPACEDIM>1)
void set0(NumPtr<Point<2> > &points,NumPtr<double> &weights,double w) {
  double xi=1./3.;
...
#if (SPACEDIM==3)
void set0(NumPtr<Point<3> > &points,NumPtr<double> &weights,double w) {
  double xi=0.25;
...
#endif
void set1(NumPtr<Point<2> > &points,NumPtr<double> &weights,
...
#if (SPACEDIM==3)
void set1(NumPtr<Point<3> > &points,NumPtr<double> &weights,
double xi_repeated,double w) {
...
void setPairs(NumPtr<Point<3> > &points,NumPtr<double> &weights,
...
#endif
void set2(NumPtr<Point<2> > &points,NumPtr<double> &weights,
...
#if (SPACEDIM==3)
void set2(NumPtr<Point<3> > &points,NumPtr<double> &weights,
double xi_0,double xi_1,double w) {
...
void set3(NumPtr<Point<3> > &points,NumPtr<double> &weights,
...
TriangleNewtonCotesQuadrature::TriangleNewtonCotesQuadrature(int np_1d){
  switch (np_1d) {
    ...
    case 4: {
      ...
      NumPtr<double> qw(quadrature_weights);
      set0(qp,qw,0.75);
      set1(qp,qw,0.,1./12.);
      break;
    }
