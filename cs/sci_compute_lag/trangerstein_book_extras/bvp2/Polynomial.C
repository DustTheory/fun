...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
void Monomial::values(double x,NumPtr<double> &v) const {
  CHECK_SAME(order+1,v.getNumber());
  v[0]=1.;
  for (int i=1;i<=order;i++) v[i]=x*v[i-1];
}

void Monomial::slopes(double x,NumPtr<double> &s) const {
  CHECK_SAME(order+1,s.getNumber());
  s[0]=0.;
  double xi=1.;
  for (int i=1;i<=order;i++) {
    s[i]=static_cast<double>(i)*xi;
    xi*=x;
  }
}

void Monomial::slope2s(double x,NumPtr<double> &s2) const {
  CHECK_SAME(order+1,s2.getNumber());
  s2[0]=0.;
  if (order>=1) s2[1]=0.;
  double xi=1.;
  for (int i=2;i<=order;i++) {
    s2[i]=static_cast<double>(i*(i-1))*xi;
    xi*=x;
  }
}

//values[j] = j'th derivative
void Monomial::derivatives(double x,int j,NumPtr<double> &values) const {
  int number_derivs=values.getNumber();
  if (number_derivs<=0) return;
  CHECK_TEST(number_derivs<=3);
  double xp=1.;
  for (int i=0;i<j-2;i++) xp*=x;
  if (number_derivs>=3) values[2]=static_cast<double>(j*(j-1))*xp;
  xp*=x;
  if (number_derivs>=2) values[1]=static_cast<double>(j)*xp;
  values[0]=xp*x;
}
...
den(ord+1),zeros(ord+1) {
  if (order>0) {
    double ro=1./static_cast<double>(order);
    for (int i=0;i<=order;i++) zeros[i]=static_cast<double>(i)*ro;
  } else {
    zeros[0]=0.;
  }
...
C0LagrangePolynomial::C0LagrangePolynomial(const NumPtr<double> &nodes) :
order(nodes.getNumber()-1),den(nodes.getNumber()),zeros(nodes.getNumber())
{
  for (int i=0;i<nodes.getNumber();i++) {
    zeros[i]=max(0.,min(1.,nodes[i]));
    if (i>0) CHECK_TEST(zeros[i]>zeros[i-1]);
  }
  for (int j=0;j<=order;j++) {
    double denj=1.;
    for (int i=0;i<=order;i++) {
...
void C0LagrangePolynomial::values(double x,NumPtr<double> &v) const {
  for (int j=0;j<=order;j++) {
    double num=1.;
    for (int i=0;i<=order;i++) {
...
void C0LagrangePolynomial::slopes(double x,NumPtr<double> &s) const {
  for (int j=0;j<=order;j++) {
    double sum=0.;
...
void C0LagrangePolynomial::slope2s(double x,NumPtr<double> &s2) const {
  for (int j=0;j<=order;j++) {
    double sum=0.;
    for (int k=0;k<=order;k++) {
      if (k==j) continue;
...
//values[j] = j'th derivative
void C0LagrangePolynomial::derivatives(double x,int j,
NumPtr<double> &values) const {
  int number_derivs=values.getNumber();
  if (number_derivs<=0) return;
  CHECK_TEST(number_derivs<=3);
  double num=1.;
...
C1LagrangePolynomial::C1LagrangePolynomial(int ord) {
  ...
  a.allocate(np1);
  b.allocate(np1);
  double ro=1./static_cast<double>(n);
  for (int i=0;i<=n;i++) zeros[i]=static_cast<double>(i)*ro;
  for (int j=0;j<=n;j++) {
    double sum=0.;
    for (int i=0;i<=n;i++) {
...
void C1LagrangePolynomial::values(double x,NumPtr<double> &v) const {
  CHECK_SAME(order+1,v.getNumber());
  int np1=n+1;
  for (int j=0;j<=n;j++) {
    double num=1.;
    ...
    double quot=num/den[j];
    v[j]=(a[j]*x+b[j])*quot;
    v[j+np1]=(x-zeros[j])*quot;
  }
}

void C1LagrangePolynomial::slopes(double x,NumPtr<double> &s) const {
  CHECK_SAME(order+1,s.getNumber());
  int np1=n+1;
  for (int j=0;j<=n;j++) {
    double num=1.;
...
void C1LagrangePolynomial::slope2s(double x,NumPtr<double> &s2) const {
  CHECK_SAME(order+1,s2.getNumber());
  int np1=n+1;
  for (int j=0;j<=n;j++) {
    double sum1=0.;
...
//values[j] = j'th derivative
void C1LagrangePolynomial::derivatives(double x,int J,
NumPtr<double> &values) const {
  int number_derivs=values.getNumber();
  if (number_derivs<=0) return;
  CHECK_TEST(number_derivs<=3);
...
void C2LagrangePolynomial::values(double x,NumPtr<double> &v) const {
  CHECK_SAME(order+1,v.getNumber());
  int np1=n+1;
  int np12=2*np1;
  for (int j=0;j<=n;j++) {
...
void C2LagrangePolynomial::slopes(double x,NumPtr<double> &s) const {
  CHECK_SAME(order+1,s.getNumber());
  int np1=n+1;
  int np12=2*np1;
  for (int j=0;j<=n;j++) {
...
void C2LagrangePolynomial::slope2s(double x,NumPtr<double> &s2) const {
  CHECK_SAME(order+1,s2.getNumber());
  int np1=n+1;
  int np12=2*np1;
  for (int j=0;j<=n;j++) {
...
//values[j] = j'th derivative
void C2LagrangePolynomial::derivatives(double x,int J,
NumPtr<double> &values) const {
  int number_derivs=values.getNumber();
  if (number_derivs<=0) return;
  CHECK_TEST(number_derivs<=3);
...
void LegendrePolynomial::values(double x,NumPtr<double> &v) const {
...
void LegendrePolynomial::slopes(double x,NumPtr<double> &s) const {
  ...
  for (int i=2;i<=order;i++) {
    r2im1+=2.;
    ri+=1.;
    double p=(r2im1*x*pm1-(ri-1.)*pm2)/ri;
    double dp=r2im1*pm1+dpm2;
    s[i]=2.*dp;
    pm2=pm1;
    pm1=p;
    dpm2=dpm1;
...
void LegendrePolynomial::slope2s(double x,NumPtr<double> &s2) const {
  ...
  for (int i=2;i<=order;i++) {
    ...
    ri+=1.;
    double p=(r2im1*x*pm1-(ri-1.)*pm2)/ri;
    double dp=(r2im1*(pm1+x*dpm1)-(ri-1.)*dpm2)/ri;
    double ddp=(r2im1*(2.*dpm1+x*ddpm1)-(ri-1.)*ddpm2)/ri;
    s2[i]=4.*ddp;
    pm2=pm1;
    pm1=p;
    dpm2=dpm1;
...
void LegendrePolynomial::derivatives(double x,int order,
NumPtr<double> &values) const {
  int number_derivs=values.getNumber();
  if (number_derivs<=0) return;
  CHECK_TEST(number_derivs<=3);
  ...
  for (int i=2;i<=order;i++) {
    r2im1+=2.;
    ri+=1.;
    double p=(r2im1*x*values[0]-(ri-1.)*pm2)/ri;
    if (number_derivs>1) {
      double dp=(r2im1*(values[0]+x*values[1])-(ri-1.)*dpm2)/ri;
      if (number_derivs>2) {
        double ddp=(r2im1*(2.*values[1]+x*values[2])-(ri-1.)*ddpm2)/ri;
        ddpm2=values[2];
        values[2]=ddp;
      }
      dpm2=values[1];
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
void HierarchicalPolynomial::values(double x,NumPtr<double> &v) const {
  int order=v.getNumber()-1;
  CHECK_TEST(order>=1);
  x=2.*x-1.;
  v[0]=0.5*(1.-x);
...
void HierarchicalPolynomial::slopes(double x,NumPtr<double> &s) const {
  int order=s.getNumber()-1;
  CHECK_TEST(order>=1);
  s[0]=-1.;
  s[1]=1.;
...
void HierarchicalPolynomial::slope2s(double x,NumPtr<double> &s) const {
  int order=s.getNumber()-1;
  CHECK_TEST(order>=1);
  s[0]=0.;
  s[1]=0.;
...
void HierarchicalPolynomial::derivatives(double x,int order,
NumPtr<double> &values) const {
  int number_derivs=values.getNumber();
  if (number_derivs<=0) return;
  CHECK_TEST(number_derivs<=3);
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
void HierarchicalSidePolynomial::values(double x,NumPtr<double> &v) const {
  int order=v.getNumber()-1;
  CHECK_TEST(order>=0);
  v[0]=-sqrt(3.);
  if (order<1) return;
  ...
  if (order<2) return;
  for (int i=2;i<=order;i++) {
    v[i]=(x*v[i-1]*sqrt(static_cast<double>(3+i*(8+4*i)))
      -v[i-2]*static_cast<double>(i-1)
          *sqrt(static_cast<double>(2*i+3)/static_cast<double>(2*i-1)))
      /static_cast<double>(i+2);
  }
}

void HierarchicalSidePolynomial::slopes(double x,NumPtr<double> &s) const {
  int order=s.getNumber()-1;
  CHECK_TEST(order>=0);
  s[0]=0.;
  if (order<1) return;
  ...
  double pm1=-sqrt(5.)*x;
  for (int i=2;i<=order;i++) {
    double a=sqrt(static_cast<double>(3+i*(8+4*i)));
    double b=static_cast<double>(i-1)
      *sqrt(static_cast<double>(2*i+3)/static_cast<double>(2*i-1));
    double c=1./static_cast<double>(i+2);
    double p=(x*pm1*a-pm2*b)*c;
...
void HierarchicalSidePolynomial::slope2s(double x,NumPtr<double> &s) const {
  int order=s.getNumber()-1;
  CHECK_TEST(order>=0);
  s[0]=0.;
  if (order<1) return;
  ...
  for (int i=2;i<=order;i++) {
    ...
    double c=1./static_cast<double>(i+2);
    double p=(x*pm1*a-pm2*b)*c;
    double dp=(a*(2.*pm1+x*dpm1)-b*dpm2)*c;
...
void HierarchicalSidePolynomial::derivatives(double x,int order,
NumPtr<double> &values) const {
  int number_derivs=values.getNumber();
  if (number_derivs<=0) return;
  CHECK_TEST(number_derivs<=3);
  ...
  for (int i=2;i<=order;i++) {
    ...
    double p=(x*pm1*a-pm2*b)*c;
    double dp=(a*(2.*pm1+x*dpm1)-b*dpm2)*c;
    double ddp=(a*(4.*dpm1+x*ddpm1)-b*ddpm2)*c;
    if (i==order) {
      values[0]=p;
...
INSTANTIATE_NUMPTR(NumPtr<double>)
