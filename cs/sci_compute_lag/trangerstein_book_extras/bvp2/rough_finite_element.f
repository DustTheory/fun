...
c***********************************************************************
      ...
      double precision x
      double precision alpha,ratio
c     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      double precision roundoff,small,huge,undefind,pi
      common/machine/roundoff,small,huge,undefind,pi
c     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c     - d/dx ( p(x) du/dx ) = 0 , u(0)=0, u(1)=1
c     where p(x)=1.5d0+sign(0.5d0,x-sqrt(0.24d0))
      alpha=sqrt(0.24d0)
      ratio=sin(pi*alpha)/(1.d0+alpha)
      if (x.lt.alpha) then
        solution=sin(pi*x)-x*ratio
      else
        solution=0.5d0*(sin(pi*x)+(1.d0-x)*ratio)
      endif
c     print *, "x = ",x
c     print *, "pi = ",pi
...
c       u(0)=left, u(1)=right
      ...
      p(x)=1.5d0+sign(0.5d0,x-sqrt(0.24d0))
      r(x)=0.d0
      f(x)=sin(pi*x)*pi**2
