...
void plotRationalInterpolant(double x1,double f1,double x2,double f2,
double x3,double f3,XYGraphTool &gt ) {
//TRACER_CALL(t,"plotRationalInterpolant");
  double dx=(B-A)/static_cast<double>(npts);
  double s32=(x3-x2)/(f3-f2);
  double s21=(x2-x1)/(f2-f1);
  double f32=(f3*x3-f2*x2)/(f3-f2);
  double f21=(f2*x2-f1*x1)/(f2-f1);
  double br=(s32-s21)/(f32-f21);
  double cr=s21-br*f21;
  double alpha=s21/s32;
  double ar=x3-f3*(x3-x1)/(f3-alpha*f1);
...
//     << (x3-ar)/(br*x3+cr) << endl;
  gt.movePen(x3,f3);
  for (double x=x3-dx;x>=gt.getLowX();x-=dx) {
    double y=(x-ar)/(br*x+cr);
    if (y<=gt.getHighY() && y>=gt.getLowY()) gt.drawLine(x,y);
    else gt.movePen(x,y);
  }
  gt.movePen(x3,f3);
  for (double x=x3+dx;x<=gt.getHighX();x+=dx) {
    double y=(x-ar)/(br*x+cr);
    if (y<=gt.getHighY() && y>=gt.getLowY()) gt.drawLine(x,y);
    else gt.movePen(x,y);
  }
...
//    draw function
	...
	xold=xnew; fold=fnew; 
        ...
        plotRationalInterpolant(xold,fold,xnew,fnew,x,fx,gt);
        double snew=(fx-fnew)/(x-xnew);
        double alpha=snew*(xnew-xold)/(fnew-fold);
        double den=fx-alpha*fold;
        xnew=x; fnew=fx;
        if (den!=0.) {
          double z=x-fx*(x-xold)/den;
//        cout << "\txold,xnew,x = " << xold << " " << xnew << " " << x
//             << endl;
//        cout << "\tfold,fnew,fx = " << fold << " " << fnew << " " << fx
//             << endl;
//        cout << "\tsold,snew,alpha = " << sold << " " << snew << " "
//             << alpha << endl;
//        cout << "\tz = " << z << endl;
          double rho=(z-min(xold,xnew))/abs(xnew-xold);
//        cout << "\trho = " << rho << endl;
          if ((0.<rho && rho<1.) && (!special || (0.1<rho && rho<0.9))) {
            cout << "\trational interpolation step" << endl;
            x=z;
          } else {
            cout << "\t rational interpolation out of bounds ==> bisection"
                 << endl;
            x=(xold+xnew)*0.5;
          }
        } else {
          cout << "\trational interpolation undefined ==> bisection"
               << endl;
          x=(xold+xnew)*0.5;
        }
        special=true;
