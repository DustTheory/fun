...
void plotQuadraticInterpolant(double x1,double f1,double x2,double f2,
double x3,double f3,XYGraphTool &gt ) {
  double dx=(B-A)/static_cast<double>(npts);
  double s32=(f3-f2)/(x3-x2);
  double s21=(f2-f1)/(x2-x1);
  double ss=(s32-s21)/(x3-x1);
  gt.movePen(x3,f3);
  for (double x=x3-dx;x>=gt.getLowX();x-=dx) {
    double y=(ss*(x-x2)+s32)*(x-x3)+f3;
...
//  setup interactive graphics
    ...
    double winsize=0.5;
    XYGraphTool gt("Muller-Traub Algorithm","x","f",A,B,fmin,fmax,&cmap,0,
      winsize);
    gt.newPage();
    gt.setbgColor("white");
    gt.setfgColor("black");
...
//    draw function
	...
	xold=xnew; fold=fnew; 
        ...
        plotQuadraticInterpolant(xold,fold,xnew,fnew,x,fx,gt);
        double snew=(fx-fnew)/(x-xnew);
        double sold=(fnew-fold)/(xnew-xold);
        double ss=(snew-sold)/(x-xold);
        double sx=snew+ss*(x-xnew);
        double disc=sqrt(sx*sx-4.*fx*ss);
        double den=(sx>0. ? sx+disc : sx-disc);
        double z=x-2.*fx/den;
        if ((z-xold)*(z-x)>0.) z=x-0.5*den/ss;
//      cout << "\txold,xnew,x = " << xold << " " << xnew << " " << x
//           << endl;
//      cout << "\tfold,fnew,fx = " << fold << " " << fnew << " " << fx
//           << endl;
//      cout << "\tsold,snew,ss,sx = " << sold << " " << snew << " "
//           << ss << " " << sx << endl;
//      cout << "\tdisc,den,z = " << disc << " " << den << " " << z << endl;
//      cout << "\tquadratic(z) = " <<  (ss*(z-xnew)+snew)*(z-x)+fx << endl;
        xnew=x; fnew=fx;
        double rho=(z-min(xold,xnew))/abs(xnew-xold);
//      cout << "\trho = " << rho << endl;
        if (!special || (0.1<rho && rho<0.9)) {
          cout << "\tMuller-Traub step" << endl;
          x=z;
        } else {
          cout << "\tbisection" << endl;
          x=(xold+xnew)*0.5;
