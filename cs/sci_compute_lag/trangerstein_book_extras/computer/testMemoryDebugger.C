...
//  If the following 3 lines are uncommented, subsequent calls to
//    mem_check_ptr(b) or mem_check will generate an error message and abort
...
//  if uncommented, the following will generate an error
//  { TRACER_CALL(t,"overrunning integer array");
//     b[2]=0;
//  }
//  mem_check_ptr(b);
...
//  uncommenting the following will generate an error,
//    because the underrun overwrites memory used by malloc
...
//  the following will not cause MemoryDebugger to abort
//    because the write out of bounds occurs beyond the buffer size
//  however, the code may get a segmentation fault as a result of the write
