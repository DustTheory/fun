...
void Vector::equ(double a,const Vector &v) {
  int n=d.size();
  F77NAME(dcopy)(n,v.d.addr(),1,d.addr(),1);
  if (a!=1.) F77NAME(dscal)(n,a,d.addr(),1);
}

double Vector::operator*(const Vector &v) const {
  int n=d.size();
  return F77NAME(ddot)(n,d.addr(),1,v.d.addr(),1);
}

double Vector::linftyNorm() const {
  int n=d.size();
  double *da=d.addr();
  int i=F77NAME(idamax)(n,da,1);
  return abs(da[i-1]);
}

void Vector::add(double a,const Vector &y) {
  int n=d.size();
  F77NAME(daxpy)(n,a,y.d.addr(),1,d.addr(),1); 
}

void Vector::swapWith(Vector &v) {
  int n=d.size();
  F77NAME(dswap)(n,d.addr(),1,v.d.addr(),1); 
}
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
LAMatrix* productOf(const LAMatrix &A,const LAMatrix &X){
  ...
  for (int j=0;j<L;j++) {
    F77NAME(dgemv)(&trans,m,n,alpha,A.addr(),m,X.addr(0,j),1,
                   beta,&((*Y)(0,j)),1);
  }
  return Y;
}
...
void switchColumns(LAMatrix &G,int i,int j) {
  int m=G.size(0),n=G.size(1);
  CHECK_BOUNDS(i,0,n)
  CHECK_BOUNDS(j,0,n)
  F77NAME(dswap)(m,&(G(0,i)),1,&(G(0,j)),1);
}
...
void switchRows(LAMatrix &G,int i,int j) {
  ...
  CHECK_BOUNDS(i,0,m)
  CHECK_BOUNDS(j,0,m)
  F77NAME(dswap)(n,&(G(i,0)),1,&(G(j,0)),1);
}
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
void Matrix::vmult(Vector &Mv,const Vector &v) const {
  ...
  double alpha=1.;
  double beta=0.;
  F77NAME(dgemv)(&trans,m,n,alpha,M.addr(),m,v.addr(),1,beta,
    Mv.addr(),1);
}
...
double Matrix::residual(Vector &r,const Vector &x,const Vector &b) const {
  ...
  CHECK_SAME(size(0),m);
  CHECK_SAME(size(1),n);
  F77NAME(dcopy)(m,b.addr(),1,r.addr(),1);
  char trans='n';
  double alpha=-1.;
  double beta=1.;
  F77NAME(dgemv)(&trans,m,n,alpha,M.addr(),m,x.addr(),1,beta,r.addr(),
    1);
  return F77NAME(dnrm2)(m,r.addr(),1);
}
