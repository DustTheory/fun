...
void MemoryNode::printOn(ostream &os) const {
  os << "MemoryNode:: nbytes=" << nbytes << ",this=" << this;
  printOrigin(os);
  os << " bk = " << bk << " fd = " << fd << endl;
}
...
void MemoryDebugger::printOn(ostream &os) {
  ...
  if (head!=0) {
    head->printOn(os); 
    register MemoryNode *mn=head->next();
    for (;mn!=head;mn=mn->next()) mn->printOn(os);
  }
}
...
#ifdef DEBUG
//printf("\n\tin operator new(size_t,nothrow) n = %d",n);
#endif
  void *p=0;
...
/*
void* operator new(size_t n,void* place) throw(std::bad_alloc) {
  return place;
}
*/
...
# endif
  {
//  if (MemoryDebugger::active()) printf("\tin __7__nw__FUl n = %d\n",n);
...
void* operator new(size_t n,const char *f,int l) {
  ...
  if (MemoryDebugger::active()) {
#ifdef DEBUG
//  printf("\n\tin operator new(size_t,char*,int) n = %d\n",n);
#endif
    p=MemoryDebugger::malloc(n,f,l);
  } else {  
    p=malloc(n);
#ifdef DEBUG
//  printf("\n\tin operator new(size_t,char*,int) n = %d p = \p\n",n,p);
#endif
  }
  return p;
}
...
//called if a user-defined placement is provided
void  operator delete(void* p,std::size_t sz) throw() {
#ifdef DEBUG
//printf("\tin operator delete(void*,size_t) p = %p\n sz = %d",p,sz);
#endif
  operator delete(p,nothrow);
}
...
//called if a user-defined placement is provided
void  operator delete(void* p,std::size_t sz,const std::nothrow_t&) throw()
{
#ifdef DEBUG
//printf("\tin operator delete(void*,size_t,nothrow) p = %p\n sz = %d",
//  p,sz);
#endif
  operator delete(p,nothrow);
}
...
/*
//called if single-object placement new when constructor throws an exception
void  operator delete(void* p,void* place) throw() {
#ifdef DEBUG
//printf("\tin operator delete(void*,void*) p = %p\n place = %p",p,place);
#endif
  operator delete(p,nothrow);
}
*/
...
//template class IDLList<MemoryNode>;
