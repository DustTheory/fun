...
double safety=1.e-3;
double tau=10.;
inline double a1(double x,double y) { return min(-safety,2.*x*x+y*y-4.); }
inline double a2(double x,double y) { return min(-safety,x*x+3.*y*y-9.); }
inline double phi(double x,double y) { return -x*x-y*y; }
inline double interiorPoint(double x,double y) {
  return phi(x,y)+tau*(-1./a1(x,y)-1./a2(x,y)
                      +1./max(safety,x)+1./max(safety,y));
}
