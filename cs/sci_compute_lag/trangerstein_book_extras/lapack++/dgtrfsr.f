c     modified from dgtrfs to multiply by matrix on the right
      subroutine dgtrfsr(trans,n,nrhs,dl,d,du,dlf,df,duf,du2,ipiv,
   ...
   70    continue
           ...
           if (kase.ne.0) then
             if (kase.eq.1) then
               call dgttrs(transt,n,1,dlf,df,duf,du2,ipiv,work(n+1),n,
     &           info)
               do i=1,n
               ...
               do i=1,n
                 work(n+i)=work(i)*work(n+i)
               enddo
               call dgttrs(transn,n,1,dlf,df,duf,du2,ipiv,work(n+1),n,
     &           info)
             endif
             goto 70
           endif
