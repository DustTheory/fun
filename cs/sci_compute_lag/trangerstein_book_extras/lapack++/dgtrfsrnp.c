/* dgtrfsrnp.f -- translated by f2c (version 20090411).
...
/* modified from dgtrfs to multiply by matrix on the right and avoid pivoting */
/* Subroutine */ int dgtrfsrnp_(char *trans, integer *n, integer *nrhs, 
	...
	doublereal *work, integer *iwork, integer *info, ftnlen trans_len)
{
	...
	*info = -13;
    ...
    if (*info != 0) {
	i__1 = -(*info);
	xerbla_("dgtrfsrnp", &i__1, (ftnlen)9);
	return 0;
    }
    if (*n == 0 || *nrhs == 0) {
	i__1 = *nrhs;
