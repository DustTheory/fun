c modified from zgtrfs to work with rows of b and x
      subroutine zgtrfsr(trans,n,nrhs,dl,d,du,dlf,df,duf,du2,ipiv,b,ldb,
     &  x,ldx,ferr,berr,work,rwork,info)
      character trans
      integer info,ldb,ldx,n,nrhs
      integer ipiv(*)
      double precision berr(*),ferr(*),rwork(*)
      complex*16 b(ldb,*),d(*),df(*),dl(*),dlf(*),du(*),du2(*),duf(*),
     &  work(*),x(ldx,*)
      integer itmax
      parameter (itmax=5)
...
c         call zlagtm(ltrans,n,1,-one,dl,d,du,x(j,1),ldx,one,work,n)
     ...
     &    count.le.itmax) then
            ...
            call zgttrs(ltrans,n,1,dlf,df,duf,du2,ipiv,work,n,info)
            if (lsame(trans,'C')) then
              do i=1,n
                work(i)=conjg(work(i))
              enddo
