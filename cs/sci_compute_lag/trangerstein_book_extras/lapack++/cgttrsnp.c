/* cgttrsnp.f -- translated by f2c (version 20090411).
...
/* modified from cgttrs to avoid pivoting */
/* Subroutine */ int cgttrsnp_(char *trans, integer *n, integer *nrhs, 
	...
	integer *info, ftnlen trans_len)
{
    ...
    /* Local variables */
    extern /* Subroutine */ int cgtts2np_(integer *, integer *, integer *, 
...
/*     intrinsic max,min */
	...
	*info = -10;
    ...
    if (*info != 0) {
	i__1 = -(*info);
	xerbla_("cgttrsnp", &i__1, (ftnlen)8);
	return 0;
    }
    if (*n == 0 || *nrhs == 0) {
	return 0;
...
/* Computing MAX */
    ...
    if (nb >= *nrhs) {
	cgtts2np_(&itrans, n, nrhs, &dl[1], &d__[1], &du[1], &b[b_offset], 
		ldb);
    } else {
	i__1 = *nrhs;
	i__2 = nb;
...
/* Computing MIN */
	    i__3 = *nrhs - j + 1;
	    jb = min(i__3,nb);
	    cgtts2np_(&itrans, n, &jb, &dl[1], &d__[1], &du[1], &b[j * b_dim1 
		    + 1], ldb);
	}
    }
    return 0;
