...
#include "Vector.C"
#ifdef DEBUG
template<> const float Vector<float,float>::undefined_ = 
...
#ifdef DEBUG
template<> const float Matrix<float,float>::undefined_ = 
  numeric_limits<float>::infinity();
#endif
...
template<> const float
  UpperHessenbergMatrix<float,float>::undefined_ =
    numeric_limits<float>::infinity();
...
const SymmetricTridiagonalMatrix<float,float> &T) : nsub(1), nt(2) {
  ...
  (*AB)(1,dim-1)=numeric_limits<float>::infinity();
}
...
template class CompanionMatrix<float,float>;
template class HadamardMatrix<float,float>;
template class HankelMatrix<float,float>;
template class HilbertMatrix<float,float>;
template class KahanMatrix<float,float>;
template class LauchliMatrix<float,float>;
template class PascalMatrix<float,float>;
template class RosserMatrix<float,float>;
template class ToeplitzMatrix<float,float>;
template class SymmetricToeplitzMatrix<float,float>;
template class VandermondeMatrix<float,float>;
template class WilkinsonMatrix<float,float>;
template void testSpecializedMatrix(float,float);
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
const Matrix<float,float> &A,Factorization::PIVOT_OPTION po) : QR(0),
tau(0),jpvt(0),piv_op(po),A_original(&A),iascl(0),ascl(float_one_),
...
//TRACER_CALL(t,"HouseholderQRFactorization::HouseholderQRFactorization");
  ...
  if (piv_op==Factorization::NO_PIVOTING) {
    ...
    int mn=min(m,n);
    int nb=F77NAME(ilaenv)(1,"SGEQRF"," ",m,n,-1,1);
    int lwkmin=mn+max(2*mn,n+1);
    int lwork=max(lwkmin,mn+2*n+nb*(n+1));
    float *work=OPERATOR_NEW_BRACKET(float,lwork);
    tau=OPERATOR_NEW Vector<float,float>(mn);
    jpvt=OPERATOR_NEW_BRACKET(int,n);
    F77NAME(sgeqp3)(m,n,QR->addr(),m,jpvt,tau->addr(),work,lwork,info);
    CHECK_SAME(info,0)
    delete [] work; work=0;
  }
}
...
//    Q' b = [ y \\ z ]:
      F77NAME(sormqr)('L','T',m,1,mn,QR->addr(),m,tau->addr(),
                      xtmp.addr(),m,&w,lwork,info);
      CHECK_SAME(info,0)
      lwork=static_cast<int>(w);
      float *work=OPERATOR_NEW_BRACKET(float,lwork);
      ...
      delete [] work; work=0;
//    solve R x' = y:
      F77NAME(strsv)('U','N','N',mn,QR->addr(),m,xtmp.addr(),1);
//    permute
      for (int i=0;i<n;i++) x[jpvt[i]-1]=xtmp[i];
      residual_norm=F77NAME(snrm2)(m-mn,xtmp.addr(mn),1);
    } else {
      CHECK_SAME(n,b.size())
      CHECK_SAME(m,x.size())
      Vector<float,float> xtmp(n);
...
//    solve R' v = P' b
      F77NAME(strsv)('U','T','N',mn,QR->addr(),m,xtmp.addr(),1);
      x=float_zero_;
      x.copyFrom(mn,xtmp);
//    Q [v\\0]
      w=numeric_limits<float>::infinity();
      lwork=-1;
      F77NAME(sormqr)('L','N',m,1,mn,QR->addr(),m,tau->addr(),
                      x.addr(),m,&w,lwork,info );
      CHECK_SAME(info,0)
      lwork=static_cast<int>(w);
      float *work=OPERATOR_NEW_BRACKET(float,lwork);
      ...
      delete [] work; work=0;
      if (mn<n) {
        residual_norm=F77NAME(snrm2)(n-mn,xtmp.addr(mn),1);
...
//    Q' B = [ Y \\ Z ]:
      F77NAME(sormqr)('L','T',m,k,mn,QR->addr(),m,tau->addr(),
                      Xtmp.addr(),m,&w,lwork,info);
      CHECK_SAME(info,0)
      lwork=static_cast<int>(w);
      float *work=OPERATOR_NEW_BRACKET(float,lwork);
...
//    P V:
      for (int j=0;j<k;j++) {
        ...
        residual_norm[j]=
          F77NAME(snrm2)(m-mn,Xtmp.addr(mn,j),1);
      }
    } else {
      CHECK_SAME(n,B.size(0))
      CHECK_SAME(m,X.size(0))
...
//    Q [V\\0]
      w=numeric_limits<float>::infinity();
      lwork=-1;
      F77NAME(sormqr)('L','N',m,k,mn,QR->addr(),m,tau->addr(),
                      X.addr(),m,&w,lwork,info );
      CHECK_SAME(info,0)
      lwork=static_cast<int>(w);
      float *work=OPERATOR_NEW_BRACKET(float,lwork);
      ...
      if (mn<n) {
        for (int j=0;j<k;j++) {
          residual_norm[j]=
            F77NAME(snrm2)(n-mn,Xtmp.addr(mn,j),1);
        }
      } else residual_norm=float_zero_;
    }
  }
...
//    cout << "\tB = " << endl;
//    R.printOn(cout);
//    cout << "\tfloat_mone_,float_one_ = " << float_mone_ << " "
//         << float_one_ << endl;
#endif
//    R=B-A'X
...
//TRACER_CALL(t,"CompleteOrthogonalDecomposition::CompleteOrthogonalDecomposition");
  ...
  int mn=min(m,n);
  int nb1=F77NAME(ilaenv)(1,"SGEQRF"," ",m,n,-1,1);
  int nb2=F77NAME(ilaenv)(1,"SGERQF"," ",m,n,-1,1);
  int nb=max(nb1,nb2);
  int lwkmin=mn+max(2*mn,n+1);
  int lwork=max(lwkmin,mn+2*n+nb*(n+1));
  float *work=OPERATOR_NEW_BRACKET(float,lwork);
  ...
  while (the_rank<mn) {
    ...
    float sminpr=numeric_limits<float>::infinity(),
      s1=numeric_limits<float>::infinity(),
      c1=numeric_limits<float>::infinity();
//                 (job,j,x,sest,w,gamma,sestpr,s,c)
    F77NAME(slaic1)(2,the_rank,xmin.addr(),smin,URV->addr(0,i-1),
      (*URV)(i-1,i-1),sminpr,s1,c1);
    float smaxpr=numeric_limits<float>::infinity(),
      s2=numeric_limits<float>::infinity(),
      c2=numeric_limits<float>::infinity();
    F77NAME(slaic1)(1,the_rank,xmax.addr(),smax,URV->addr(0,i-1),
  ...
  if (the_rank<n) {
    vtau=OPERATOR_NEW Vector<float,float>(the_rank);
    w=numeric_limits<float>::infinity();
    int lwork=-1;
    F77NAME(stzrzf)(the_rank,n,URV->addr(),m,vtau->addr(),&w,lwork,info);
    CHECK_SAME(info,0)
    lwork=static_cast<int>(w);
    work=OPERATOR_NEW_BRACKET(float,lwork);
    F77NAME(stzrzf)(the_rank,n,URV->addr(),m,vtau->addr(),work,lwork,
      info);
    CHECK_SAME(info,0)
    delete [] work; work=0;
  }
...
//TRACER_CALL(t,"CompleteOrthogonalDecomposition::leftOrthogonalPart");
  ...
  int lwork=-1;
  int info;
  F77NAME(sorgqr)(m,m,the_rank,Q->addr(),m,utau->addr(),&w,lwork,info);
  CHECK_SAME(info,0)

  lwork=static_cast<int>(w);
  float *work=OPERATOR_NEW_BRACKET(float,lwork);
  F77NAME(sorgqr)(m,m,the_rank,Q->addr(),m,utau->addr(),work,lwork,info);
  CHECK_SAME(info,0)
  delete [] work; work=0;
  return Q;
}
...
template<> OrthogonalMatrix<float,float>* 
CompleteOrthogonalDecomposition<float,float>
...
//TRACER_CALL(t,"CompleteOrthogonalDecomposition::rightOrthogonalPartTransposed");
  ...
  int lwork=-1;
  int info;
  F77NAME(sormrz)('L','T',n,n,the_rank,n-the_rank,URV->addr(),m,
...
//  U' b = [ y \\ z ]:
    ...
    delete [] work; work=0;
//  solve R v = y:
    F77NAME(strsv)('U','N','N',the_rank,URV->addr(),m,xtmp.addr(),1);
    Vector<float,float> px(n,float_zero_);
    px.copyFrom(the_rank,xtmp);
    float w=numeric_limits<float>::infinity();
    lwork=-1;
//  P'x=V[v\\0]:
    F77NAME(sormrz)('L','T',n,1,the_rank,n-the_rank,URV->addr(),m,
...
//  permute
    ...
    residual_norm=F77NAME(snrm2)(m-the_rank,xtmp.addr(the_rank),1);
...
//  P'b:
    ...
    lwork=-1;
//  [y\\z]=V'(P'b):
    F77NAME(sormrz)('L','N',n,1,the_rank,n-the_rank,URV->addr(),m,
    ...
    delete [] work; work=0;
//  solve R' v = b
    F77NAME(strsv)('U','T','N',the_rank,URV->addr(),m,xtmp.addr(),1);
...
//  U [v\\0]
    ...
    CHECK_SAME(info,0)
    delete [] work; work=0;
    residual_norm=F77NAME(snrm2)(n-the_rank,xtmp.addr(the_rank),1);
  }
  if (iascl!=0) {
    F77NAME(slascl)('G',0,0,anrm,ascl,scllen,1,x.addr(),x.size(),info);
    CHECK_SAME(info,0)
...
//  U' B = [ Y \\ Z ]:
    ...
    delete [] work; work=0;
//  solve R V = Y:
    F77NAME(strsm)('L','U','N','N',the_rank,k,float_one_,URV->addr(),m,
      Xtmp.addr(),m);
    ...
    lwork=-1;
//  P'X=V[V\\0]:
    F77NAME(sormrz)('L','T',n,k,the_rank,n-the_rank,URV->addr(),m,
      vtau->addr(),PX.addr(),n,&w,lwork,info);
    CHECK_SAME(info,0)
    lwork=static_cast<int>(w);
    work=OPERATOR_NEW_BRACKET(float,lwork);
...
//  permute
    for (int j=0;j<k;j++) {
      ...
      residual_norm[j]=F77NAME(snrm2)(m-the_rank,Xtmp.addr(the_rank,j),1);
    }
  } else { // min || X || min || B - A' X ||
...
//  P'B:
    ...
    lwork=-1;
//  [Y\\Z]=V'(P'B):
    F77NAME(sormrz)('L','N',n,k,the_rank,n-the_rank,URV->addr(),m,
      vtau->addr(),Xtmp.addr(),n,&w,lwork,info);
    CHECK_SAME(info,0)
    lwork=static_cast<int>(w);
    float *work=OPERATOR_NEW_BRACKET(float,lwork);
    ...
    delete [] work; work=0;
//  solve R' V = B
    F77NAME(strsm)('L','U','T','N',the_rank,k,float_one_,URV->addr(),m,
      Xtmp.addr(),n);
...
//  U [V\\0]
    ...
    delete [] work; work=0;
    for (int j=0;j<k;j++) {
      residual_norm[j]=F77NAME(snrm2)(n-the_rank,Xtmp.addr(the_rank,j),1);
    }
  }
  if (iascl!=0) {
    F77NAME(slascl)('G',0,0,anrm,ascl,scllen,k,X.addr(),X.size(0),info);
...
//template<> float VirtualLinearProgram<float>::huge_ =
//  numeric_limits<float>::infinity();
