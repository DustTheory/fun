c modified from zgttrs to avoid pivoting
      subroutine zgttrsnp(trans,n,nrhs,dl,d,du,b,ldb,info)
      character trans
     ...
     &.and. .not.( trans.eq.'C' .or. trans.eq.'c' ) ) then
      ...
      if (nb.ge.nrhs) then
        call zgtts2np(itrans,n,nrhs,dl,d,du,b,ldb)
      else
        do j=1,nrhs,nb
          jb=min(nrhs-j+1,nb)
          call zgtts2np(itrans,n,jb,dl,d,du,b(1,j),ldb)
        enddo
      endif
      end
