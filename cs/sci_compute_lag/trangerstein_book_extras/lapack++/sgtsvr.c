/* sgtsvr.f -- translated by f2c (version 20090411).
...
/* modified from sgtsv to solve for rows of B, rather than columns */
/* Subroutine */ int sgtsvr_(integer *n, integer *nrhs, real *dl, real *d__, 
	real *du, real *b, integer *ldb, integer *info)
{
	...
	*info = -7;
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
		...
		du[i__] = temp;
		temp = b[i__ * b_dim1 + 1];
		b[i__ * b_dim1 + 1] = b[(i__ + 1) * b_dim1 + 1];
		b[(i__ + 1) * b_dim1 + 1] = temp - fact * b[(i__ + 1) * 
			b_dim1 + 1];
	    }
	}
	if (*n > 1) {
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
		...
		for (j = 1; j <= i__2; ++j) {
		    temp = b[j + i__ * b_dim1];
		    b[j + i__ * b_dim1] = b[j + (i__ + 1) * b_dim1];
		    b[j + (i__ + 1) * b_dim1] = temp - fact * b[j + (i__ + 1) 
			    * b_dim1];
		}
	    }
	}
