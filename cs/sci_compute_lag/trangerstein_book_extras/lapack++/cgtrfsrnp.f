c modified from cgtrfsr to avoid pivoting
      subroutine cgtrfsrnp(trans,n,nrhs,dl,d,du,dlf,df,duf,b,ldb,
     &  x,ldx,ferr,berr,work,rwork,info)
      character trans
      integer info,ldb,ldx,n,nrhs
      real berr(*),ferr(*),rwork(*)
      complex b(ldb,*),d(*),df(*),dl(*),dlf(*),du(*),duf(*),
     &  work(*),x(ldx,*)
      integer itmax
      parameter (itmax=5)
      real zero,one
...
c         call zlagtm(ltrans,n,1,-one,dl,d,du,x(j,1),ldx,one,work,n)
     ...
     &    count.le.itmax) then
            ...
            call cgttrsnp(ltrans,n,1,dlf,df,duf,work,n,info)
            if (lsame(trans,'C')) then
              do i=1,n
                work(i)=conjg(work(i))
              enddo
