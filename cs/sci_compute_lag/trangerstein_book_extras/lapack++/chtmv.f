      subroutine chtmv(n,alpha,L,D,x,incx,beta,y,incy)
c     chtmv performs y := A * x * alpha + y * beta
c     where alpha and beta are scalars, x and y are vectors and
c     A is a Hermitian tridiagonal matrix with
...
c     incy is the stride for y
      ...
      if (n.eq.2) then
        y(1     )=y(1     )*beta+(D(1)*x(1)+conjg(L(1))*x(1+incx))*alpha
